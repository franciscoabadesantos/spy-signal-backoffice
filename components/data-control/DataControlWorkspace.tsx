...
const DOMAINS = [
  ...
  ['filings', 'Filings'], ['corporateActions', 'Actions'], ['investorEvents', 'Investor events'], ['guidance', 'Guidance'],
] as const
...
export function DataControlWorkspace({ adminEmail }: Props) {
  ...
  return (
    <div className="page-stack">
      <section className="card">
        <div className="split-row">
          <div>
            ...
            <h1>Data Control</h1>
            <p className="small">Coverage and source health for the tracked universe. Raw observations are available only through domain drill-down.</p>
          </div>
      ...
      ) : <SourcesView sources={sources} loading={loading} />}
    </div>
  )
}
...
function SourcesView({ sources, loading }: { sources: Sources; loading: boolean }) {
  return <section className="card"><div className="split-row"><div><h2>Source health</h2><p className="small">Primary sources are table columns; contributing sources come from canonical provenance metadata.</p></div><Availability available={sources.available} /></div>
...
function DomainCell({ symbol, domain, snapshot }: { symbol: string; domain: string; snapshot: DomainSnapshot | undefined }) {
  const state = snapshot?.state ?? 'unavailable'
  const href = RAW_PATHS[domain] ? `${RAW_PATHS[domain]}?symbol=${encodeURIComponent(symbol)}` : null
  return <td><span className={`badge ${badgeClass(state)}`}>{state}</span><div className="small">{snapshot?.recordCount ?? 0} rows</div><div className="small">{snapshot?.latestKnownAt ?? '-'}</div>{href && snapshot?.recordCount ? <Link className="small" href={href}>Inspect</Link> : null}</td>
}
...
function badgeClass(value: string): string { return value === 'healthy' ? 'completed' : value === 'missing' || value === 'unavailable' ? 'failed' : 'muted' }
