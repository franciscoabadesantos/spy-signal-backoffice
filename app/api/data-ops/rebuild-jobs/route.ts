...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${backendBaseUrl()}/analyst/data-ops/rebuild-jobs`, {
      method: 'POST',
...
export async function GET(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${backendBaseUrl()}/analyst/data-ops/rebuild-jobs${query ? `?${query}` : ''}`, {
      method: 'GET',
      headers: backendHeaders(),
      cache: 'no-store',
    })
